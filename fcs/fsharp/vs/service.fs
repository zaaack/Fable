...
module EnvMisc =
    ...
    let maxTimeShareMilliseconds = int64 (GetEnvInteger "FCS_MaxTimeShare" 50)
...
type FSharpCheckProjectResults(keepAssemblyContents:bool, errors: FSharpErrorInfo[], details:(TcGlobals*TcImports*CcuThunk*ModuleOrNamespaceType*TcSymbolUses list*TopAttribs option*CompileOps.IRawFSharpAssemblyData option * ILAssemblyRef * AccessorDomain * TypedImplFile list option * string list) option, reactorOps: IReactorOperations) =
...
#if FABLE_COMPILER
    new (keepAssemblyContents, errors, details, reactorOps, _) = FSharpCheckProjectResults(keepAssemblyContents, errors, details, reactorOps)
#endif
...
type FSharpCheckFileResults(errors: FSharpErrorInfo[], scopeOptX: TypeCheckInfo option, dependencyFiles: string list, builderX: IncrementalBuilder option, reactorOpsX:IReactorOperations) =
...
#if FABLE_COMPILER
    new (errors, scopeOptX, dependencyFiles, builderX, reactorOpsX, _) = FSharpCheckFileResults(errors, scopeOptX, dependencyFiles, builderX, reactorOpsX)
#endif
    ...
    member info.GetVisibleNamespacesAndModulesAtPoint(pos: pos) : Async<ModuleOrNamespaceRef []> = 
        reactorOp "GetVisibleNamespacesAndModulesAtPoint" [| |] (fun _ctok scope -> scope.GetVisibleNamespacesAndModulesAtPosition(pos) |> List.toArray)
            ...
            [ for (nm,opts) in options.ReferencedProjects ->
                { new IProjectReference with 
                        ...
                        member x.GetLogicalTimeStamp(cache) = 
                            self.GetLogicalTimeStampForProject(cache, ctok, opts, ct)
...
//            | Some (parseResults, checkResults, _, _) when builder.AreCheckResultsBeforeFileInProjectReady(filename) -> 
    ...
    member private bc.GetLogicalTimeStampForProject(cache, ctok, options, ct) =
    ...
    member bc.CheckProjectInBackground (options) =
        ...
        reactor.SetBackgroundOp (Some (fun ctok -> 
            ...
            match builderOpt with 
            | None -> false
            | Some builder -> builder.Step(cache, ctok, ct)))
