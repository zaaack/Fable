...
type internal IncrementalBuilder = 
      ...
      member Step : TimeStampCache * CompilationThreadToken * ct: CancellationToken -> bool
      ...
      member GetLogicalTimeStampForProject: cache: TimeStampCache * CompilationThreadToken -> DateTime
...
module internal IncrementalBuild =
    ...
    val Eval : TimeStampCache -> CompilationThreadToken -> (CompilationThreadToken -> PartialBuild -> unit) -> CancellationToken -> INode -> PartialBuild -> PartialBuild

    /// Evaluate a build for a vector up to a limit. Only required for unit testing.
    val EvalUpTo : TimeStampCache -> CompilationThreadToken -> (CompilationThreadToken -> PartialBuild -> unit) -> CancellationToken -> INode * int -> PartialBuild -> PartialBuild

    /// Do one step in the build. Only required for unit testing.
    val Step : TimeStampCache -> CompilationThreadToken -> (CompilationThreadToken -> PartialBuild -> unit) -> CancellationToken -> Target -> PartialBuild -> PartialBuild option
